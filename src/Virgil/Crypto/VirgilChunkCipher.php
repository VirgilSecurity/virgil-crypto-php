...
class VirgilChunkCipher extends VirgilCipherBase
{
    ...
    function encrypt($source, $sink, $embedContentInfo = true, $preferredChunkSize = null)
    {
        switch (func_num_args()) {
            case 2:
            case 3:
                VirgilChunkCipher_encrypt($this->_cPtr, $source, $sink, $embedContentInfo);
                break;
            default:
                VirgilChunkCipher_encrypt($this->_cPtr, $source, $sink, $embedContentInfo, $preferredChunkSize);
        }
    }
    ...
    function decryptWithKey($source, $sink, $recipientId, $privateKey, $privateKeyPassword = null)
    {
        switch (func_num_args()) {
            case 4:
                VirgilChunkCipher_decryptWithKey($this->_cPtr, $source, $sink, $recipientId, $privateKey);
                break;
            default:
                VirgilChunkCipher_decryptWithKey(
                    $this->_cPtr,
                    $source,
                    $sink,
                    $recipientId,
